...
class CircularUpsample(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, dilation=1):
        ...
        self.true_padding = (dilation[0] * (kernel_size[0] - 1) - padding[0], dilation[1] * (kernel_size[1] - 1) - padding[1], dilation[2] * (kernel_size[2] - 1) - padding[2])
        # this ensures that no padding is applied by the ConvTranspose3d layer since we manually apply it before
        self.removed_padding = (
            dilation[0] * (kernel_size[0] - 1) + stride[0] + padding[0] - 1,
...
class Unet3D(nn.Module):
    def __init__(
        ...
        temporal_attn = lambda dim: EinopsToAndFrom(
            "b c f h w", "b (h w) f c", Attention(dim, heads=attn_heads, dim_head=attn_dim_head, rotary_emb=rotary_emb, cond_dim=self.cond_dim)
        )
        ...
        if self.padding_mode == "zeros" or self.padding_mode == "circular":
            self.init_conv = nn.Conv3d(
                self.input_channels, init_dim, (1, init_kernel_size, init_kernel_size), padding=(0, init_padding, init_padding), padding_mode=self.padding_mode
            )
        else:
    ...
    def forward(self, x, time, x_self_cond=None, cond=None, null_cond_prob=0.0):
...
if __name__ == "__main__":
    ...
    model = Unet3D(dim=16, channels=7, dim_mults=(1, 1, 2), attn_dim_head=16, sigmoid_last_channel=False)
    ...
    def dict_in(resolution):
        x = torch.randn((1, 7, 128, 128))
        time = torch.randn((1,))
        return dict(x=x, time=time)

    macs, params = get_model_complexity_info(
        model, (3, 128, 128), input_constructor=dict_in, as_strings=True, backend="pytorch", flops_units="MMac", print_per_layer_stat=True, verbose=True
    )
